...
from reconsthdr.dataset.hdr import convert_ldr_to_tensor
from reconsthdr.models import model_factory
from reconsthdr.utils.hdr_calibration import (calibrate_hdr_with_linear_rgb,
                                              sRGB2linearRGB)
...
class Predictor:
    ...
    def __call__(self, ldr: np.ndarray) -> np.ndarray:
        linear_ldr = sRGB2linearRGB(ldr)
        ldr_tensor = convert_ldr_to_tensor(linear_ldr)
        with torch.no_grad():
            ldr_tensor = ldr_tensor.unsqueeze(0).to(self.device)
            hdr = np.exp(self.net(ldr_tensor).cpu().permute(0, 2, 3, 1).numpy()[0])
            return calibrate_hdr_with_linear_rgb(hdr, linear_ldr)
